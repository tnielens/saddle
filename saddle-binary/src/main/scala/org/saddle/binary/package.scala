...
/** Binary serialization for Frame[String,String,T] or Mat[T] with primitive T
  ...
  *     - rowmajor : a boolean, indicating whether the data is stored in
  *       row-major or col-major order
  *     - datatype : string, either "double", "long", "int", "float", "byte"
  *   - The header is padded with spaces (0x20) such that HEADER_LENGTH+12 is
  *     divisible by 16. The count of spaces are included in HEADER_LENGTH.
  *   - The next width * numRows * numCols bytes form a little endian primitive
  *     array in row-major or col-major order. numRows and numCols are
  *     determined from the rowix/numrows and colix/numcols header fields. width
  *     is determined from the datatype field (8 for double and long, 4 for int
  *     and float, 1 for byte)
  */
