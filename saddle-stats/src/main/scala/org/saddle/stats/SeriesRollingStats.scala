...
/** Rolling statistical methods made available on numeric Series objects via
  * enrichment. These methods scan over the Series and compute cumulative
  * values.
  */
class SeriesRollingStats[X: ST: ORD, T: Vec2RollingStats: ST](s: Series[X, T]) {
  protected val ev = implicitly[Vec2RollingStats[T]]

  /** Rolling count; compute count of number of elements in Series over a
  ...
  /** Rolling sum; compute sum of elements in Series over a sliding window,
  ...
  /** Rolling mean; compute mean of elements in Series over a sliding window,
    * ignoring any NA values.
    * @param winSz
    *   Size of the sliding window
    */
  ...
  /** Rolling median; compute mean of elements in Series over a sliding window,
    * ignoring any NA values.
    * @param winSz
    *   Size of the sliding window
    */
...
object SeriesRollingStats {
  ...
  /** Factory method for creating an enriched Series object containing
    * statistical functions; usually created implicitly.
    *
    * @param s
    *   Series to wrap
    * @tparam X
